...
> Proposta · `2026-09-17` · nascida da prática, num projeto real de documentação de produto.
...
> O primeiro uso foi um produto de saúde. O método tem de ser replicável em outro projeto sem reescrever nada.

## O ciclo

```
Escrever (agente + dono)  →  Publicar sob demanda  →  Revisar (pessoas, no site)
        ↑                                                     │
...
| Etapa | Quem | Como |
|---|---|---|
| **Escrever** | agente com o dono | páginas no padrão aprovado (`PADRAO-DOCUMENTO.md`), caixas numeradas, lições (`LICOES-DE-REVISAO.md`) |
| **Publicar** | o dono decide quando | `gh workflow run publicar-leitura.yml` — junta mudanças, publica de uma vez |
| **Revisar** | revisores liberados (ex.: a especialista do domínio) | no site, em cada caixa: **Aprovar · Pedir alteração · Comentar**; responder decisões em aberto |
| **Aplicar** | **só o agente**, com o dono | lê o pedido, **analisa o impacto** (termo em outras páginas, caso de uso que outras partes usam, aprovações que cairão), pergunta o que for ambíguo, aplica, commita com `Pedido:` e `Solicitado-por:`, marca o pedido como aplicado |
...
3b. **Aprovar é de owner e admin** (decisão de desenho, `2026-09-17`). O ✓ deles não é opinião: vira **trava
   ...
   nenhum admin além do owner — na prática, quem aprova é o dono.*
...
4. **Toda alteração nasce de um pedido ou de uma sessão com o dono**, e o commit diz qual.
...
- Página de triagem: a fila de pedidos do projeto.

## Papéis do sistema (`2026-09-17`)
...
|---|---|---|
| **owner** | metodologia | **um, sempre o mesmo** — o arquiteto fundador (o dono do projeto). Tudo, inclusive criar as roles |
| **admin** *(papel do projeto)* | role do projeto, com a capacidade de aprovar | tudo o que o owner faz, **menos ser owner** ou trocar quem é. Hoje: nenhum |
| **especialista** *(papel do projeto)* | role com a tag `founder` — quem responde pelo domínio | ver a Fundamental inteira, pedir alteração, comentar, responder decisão |
...
- Hoje não existe admin nenhum, então "só o dono aprova" continua verdade sem precisar de exceção no código.

**Da metodologia, só `owner` e `founder`** (`2026-09-17`). **`admin`, `gestor médico`, `operador`,
`auditor` — esses são papéis DO PROJETO**, de quem aplica o método. Quem adotar o Doc First noutro
...
| **role** | do projeto | dado que o owner cria e nomeia. Num projeto de saúde, por exemplo: admin, gestor médico, gestor, operador, médico, enfermeiro, auditor |
| **capacidade** | metodologia define, projeto atribui | **ver · pedir · aprovar**. A tag `founder` dá ver e pedir; **aprovar** é do owner e de quem ele der. Num projeto de saúde, quem aprova é o `admin` |
...
- ⚠️ **Gargalo conhecido (`2026-09-17`):** hoje aprovar **e** implementar passam pelo owner. O
...
- **O motor só conhece owner, admin e "outro".** O papel de **produto** de quem não é admin — Gestor,
  os papéis de negócio de cada produto — são DO PROJETO, e costumam vir do provedor de identidade.
  Se o motor nomeasse papel de produto, deixaria de ser motor.

## Como o ✓ do dono volta para o repositório

O dono **valida no site, não no terminal** (decisão de desenho, `2026-09-17`): "a validação acontece no site;
aqui a gente só constrói". O site não escreve no repositório — quem fecha o caminho é o agente:

```
dono clica ✓ no trecho ──► evento `aprovacao` no Firestore ──► agente: doc-first sincronizar
                                                          ──► docs/validacoes.json + data-validado no HTML
```

- `node review/cli/doc-first.ts sincronizar` — roda no início de toda sessão (a rotina de retomada chama).
...
4. **Perguntar ao dono** o que for ambíguo ou tocar caixa validada ("só nesta caixa ou nas outras N?"). Se a decisão for do revisor, `estado ID aguardando "pergunta"`.
5. **Aplicar** no repositório, rodar `marcar_ids.py` se criou caixa, `doc-first conferir` (caixa validada alterada só com ok do dono — revalidar), conferir prints.
...
9. **Publicar** só quando o dono pedir — até lá, o revisor vê "Aplicado" mas o texto novo ainda não está no site.
...
## Motor e conteúdo — separar desde agora (`2026-09-17`)

O Doc First não é de nenhum projeto: é **motor**. A intenção é empacotar e distribuir para a comunidade, com o
o primeiro projeto como caso de uso, não como dono. Por isso a fronteira se marca **enquanto se
...
| Marca | — | tema, ícones, nome do produto, tipografia |

**Regra de quem escreve código aqui:** nada do motor pode citar um projeto, um cliente, linha de cuidado ou um
e-mail — o que varia vira configuração (como `REVISAO_OWNER` e `REVISAO_ADMINS` já são). Quando algo do
motor precisar saber do produto, o produto passa o valor; o motor não vai buscar.

⚠️ **Sem pesquisa de metodologias parecidas** antes de o motor existir — construir primeiro,
...
6. **Método empacotado** — modelo de repositório e guia, com o motor já separado do conteúdo
